...
def measure_qrs_duration(ecg, locs, fs):
    ...
    for loc in locs:
        ...
        peak_val = segment[search_radius]  # The R-peak location
        threshold = baseline + 0.20 * abs(peak_val - baseline)
...
def analyze_p_wave(ecg, locs, fs):
    ...
    results = []
    if i > 0:
    prev_rr_samples = locs[i] - locs[i-1]
    pre_start = min(int(0.25 * fs), int(prev_rr_samples * 0.4))
...
def classify_ectopic_beats_full(RR, t_rr, locs, t, ecg, fs):
    ...
    for i in range(window_size, len(RR) - 1):
        ...
        elif qrs_ms >= 130 and t_inverted:
...
def detect_af_candidates(RR, t_rr, locs, t):
    ...
    for i in range(window_size, len(RR) - 1):
        ...
        if RR[i] < 0.80 * local_median:
            ...
            idx= np.where(np.abs(t[locs] - pac_time) < pd.Timedelta(milliseconds=1))[0]
            if len(idx) > 0:
